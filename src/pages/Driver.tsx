    ...
    <Layout title="Minhas Entregas">
      ...
      <AnimatePresence>
        {selected && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-slate-950/50 backdrop-blur-sm" onClick={() => !finishing && setSelected(null)}>
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 28 }} onClick={(event) => event.stopPropagation()} className="absolute bottom-0 left-4 right-4 mx-auto max-h-[92dvh] max-w-2xl overflow-y-auto rounded-t-[2rem] bg-white px-5 pb-[calc(1.5rem+env(safe-area-inset-bottom))] pt-6 shadow-2xl sm:left-0 sm:right-0 sm:rounded-3xl sm:p-8 sm:pb-8">
            <div className="mx-auto mb-5 h-1.5 w-12 rounded-full bg-slate-200" />
            <button onClick={() => setSelected(null)} disabled={finishing} className="absolute right-4 top-4 rounded-xl p-2.5 text-slate-400 hover:bg-slate-100"><X className="h-5 w-5" /></button>
